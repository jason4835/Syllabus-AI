/**
 * Every degree-granting US institution the open `university-domains-list`
 * dataset knows about -- 2338 names, US entries only, nothing else
 * from the file kept.
 *
 * GENERATED. Do not hand-edit; regenerate from the source instead:
 *
 *   curl -sL https://raw.githubusercontent.com/Hipo/university-domains-list/master/world_universities_and_domains.json \\
 *     | node -e "const a=JSON.parse(require('fs').readFileSync(0));console.log([...new Set(a.filter(u=>u.country==='United States').map(u=>u.name.trim()))].sort().join('\\n'))"
 *
 * Source: github.com/Hipo/university-domains-list (MIT). Names are stored
 * exactly as the dataset spells them, because that spelling IS the canonical
 * value `@/lib/schools` stores -- see there for aliases and matching.
 *
 * Server-only by size (~74 KB): the onboarding card
 * searches it through `GET /api/schools`, never by importing it.
 */
export const US_SCHOOL_NAMES: readonly string[] = [
  "42 US",
