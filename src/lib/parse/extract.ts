...
1. EVERY dated graded item becomes one assessment: problem sets, homework, quizzes, midterms, the final exam, labs, papers, projects, project milestones, presentations, and graded reading responses. A syllabus that lists a deadline twice (once in a summary table, once in the weekly schedule) still yields ONE assessment. Do not invent items that are only discussed in prose ("exams are closed-book" is not an exam).
   ...
   - TWO DATES, TWO ITEMS. A deliverable with two dated parts ("your exhibit is due Friday, December 17 and your response to a colleague's exhibit is due Monday, December 21") is two assessments, each on its own date, titled so both are recognisable ("Final Project: exhibit", "Final Project: response to a colleague's exhibit"). One item with one date silently drops the other deadline.
   - A GRADED DISCUSSION BOARD, forum post, discussion starter or reading response is kind "assignment", never "reading". "reading" is for assigned reading only -- pages to read, not work to submit.
   - A POINT VALUE IS A GRADED ITEM. A line in the assignment list that states points or a count ("Twenty (20) Conversation Continuers, two (2) per week, 2 points/week") is graded work even with no percentage and no date: emit it, undated, with the rule in notes. Leaving it out loses a fifth of the student's work.
...
7. MEETING TIMES are every recurring meeting the syllabus states. daysOfWeek uses 0 = Sunday through 6 = Saturday; startTime and endTime are 24-hour HH:MM.
   ...
   - ASYNCHRONOUS IS NOT A MEETING. "Mondays (asynchronous online) and Thursdays 12:15 to 1:40 (in person)" is ONE meeting, on Thursday. A day the document calls asynchronous has no time to state and none to ask for, so emit nothing for it and say so in a warning. A course that is entirely online and asynchronous has no class meetings; its office hours are STILL EMITTED, with kind "office_hours", exactly as for any other course.
   ...
   - OFFICE HOURS ARE MEETING TIMES. Extract them explicitly, with kind "office_hours", instructor set to whose hours they are, location set to where they are held, and section null. If the syllabus lists hours for several people -- the instructor and one or more TAs -- emit ONE entry per person per pattern. A bare "by appointment" with no day and no time is not a meeting; do not emit anything for it. But hours held on stated days at stated times ARE emitted, even when the document adds "by appointment only", "virtual", "via Zoom", "in person" or "Web Ex" to them -- "By appointment only, Wednesday 11:00 am-2:00 pm" is a Wednesday 11:00-14:00 office hour.
