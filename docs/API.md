...
| `/api/schools` | GET | `q` (2–80 chars) | `{ schools: string[] }` — up to 8 canonical school names, prefix matches first, searched by name and alias ("nyu" finds New York University). Unauthenticated: the list is public and the search is in-memory. Feeds the onboarding card's typeahead. |
| `/api/me/profile` | POST | `{ school?: string; year?: "freshman"\|"sophomore"\|"junior"\|"senior"\|"grad"\|"other"; source?: "search"\|"social"\|"friend"\|"ad"\|"professor"\|"other" }` or `{ skip: true }` | `{ profile: UserProfile }` — the one-time onboarding card. `school` is **canonicalised** against `src/lib/schools.ts` and only the list's spelling is stored; text that matches nothing is kept as `schoolOther` and never mixed into `school`. Both answer and skip set `profile.completedAt`, which is what hides the card. Unknown enum values are dropped, `school` is capped at 80 chars. **403** for a demo sandbox. |
...
| `/api/upload` | POST | `multipart/form-data`, field `file` (PDF); optional fields `replace` (course id), `allowDuplicate` (`1`), `termId` (file the parsed course under an existing term) or `newTerm` (`{ name; termType; startDate; endDate }` to create one first). **Or** `pendingId` with **no file**: replays a parse the paywall refused earlier (see the 402 below) through the identical create path — no model call, takes the `edit` budget rather than the upload budget | `{ courseId: string; course: Course; assessments: Assessment[]; warnings: string[]; replaced: string \| null }` — **409** `{ duplicateOf: { id; code; title; term } }` when the parsed course matches an existing one by code (case/space-insensitive) and term, unless `replace=<thatId>` (the old course and its assessments are deleted after the new one is saved; `replaced` carries the old id) or `allowDuplicate=1`. If neither `termId` nor `newTerm` is given, the server infers or creates a term itself (see `docs/TERM-PASS.md`). **402** — see "The Term Pass paywall" below — when the resolved term already holds its free course and is not premium. The parse is **kept** (`pending_uploads`) and the 402's `paywall.term.pendingUpload` names it (`{ id; fileName; courseCode; courseTitle; assessmentCount }`); once the term reads premium the client replays it with `pendingId` and the course is created without the file being sent again. |
...
| `/api/terms` | GET | — | `{ terms: (AcademicTerm & { courseCount: number; access: "free" \| "premium" \| "expired"; pendingUpload: PendingUploadSummary \| null })[] }` — every term the caller owns. `pendingUpload` is the syllabus (if any) parsed for that term and refused with the paywall, waiting to be added once the term is unlocked. `access` is `"premium"` while the term's grace period (14 days past its end date) hasn't run out, `"expired"` for a term that was premium and now is past that, `"free"` otherwise. |
...
| `/api/sync` | POST | `{ courseId?: string; dryRun?: boolean }`; send `Accept: application/x-ndjson` to **stream** progress | `CalendarSyncResult` — with the NDJSON `Accept`, the body is one `{"progress":{done,total}}` line per event written or removed, then the ordinary `ApiResult` envelope as the final line (HTTP 200 even on failure, since the status is sent before the first progress byte; the failure is in the final envelope). Without it, plain JSON as before. Also honours `calendarPrefs`; removes events whose source was deleted or deselected; `needsSection` lists courses with at least one section question still unanswered (only that question's meetings are skipped, never guessed — an answered lecture still syncs while the lab question is open) |
